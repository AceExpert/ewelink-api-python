...
class Ewelink(Constants):
    ...
    def setDevicePowerState(self, id : str, state : str, channel = 1):
        ...
        res = self.__make_request(method='POST', uri="/user/device/status", json = {'deviceid': id, 'params': {'switch': state}, 'appid': self.APP_ID, 'nonce': self.__nonce(), \
    ...
    def __get_raw_creds(self, password : str, email=None, phone=None) -> list:
        creds={'appid':self.APP_ID, 'password':password, 'ts': int(time.time()), 'version':6, 'nonce':self.__nonce(), \
            'os': 'iOS', 'model': 'iPhone10,6', 'romVersion': '11.1.2', 'appVersion': '3.5.3', 'imei': str(uuid.uuid4())}
        if (
           not email 
           or not re.match(r'^[a-zA-Z0-9.]+@[a-zA-Z\-]+\.[a-zA-Z]+(?:(?:\.[a-zA-Z]+)+?)?$', email, re.I) 
           and re.match(r'^(?:\+\d{,4})?\d{10}$', str(phone).replace(' ', '').replace('(', '').replace(')', ''))
        ):
